...
def analyze_policy(task: str, chunks: list) -> dict:
    ...
    try:
        ...
        prompt = f"""
Bạn là chuyên gia phân tích chính sách (Policy Analyst). 
Nhiệm vụ: Dựa vào câu hỏi và tài liệu đi kèm, xác định xem yêu cầu có vi phạm chính sách hoặc rơi vào trường hợp ngoại lệ (exception) không.
...
Tiêu chí kiểm tra các ngoại lệ:
1. Flash Sale: Không được hoàn tiền.
2. Sản phẩm kỹ thuật số (License key, Subscription): Không được hoàn tiền.
3. Sản phẩm đã kích hoạt/đã sử dụng: Không được hoàn tiền.
4. Thời điểm mua: Nếu trước 01/02/2026, áp dụng chính sách v3 (Lưu ý: docs hiện tại là v4).

Hãy trả về kết quả dưới định dạng JSON duy nhất:
{{
    "policy_applies": <bool, True nếu được chấp nhận theo chính sách ví dụ như được hoàn tiền>,
    "policy_name": "refund_policy_v4",
    "exceptions_found": [
        {{"type": "tên_ngoại_lệ", "rule": "nội_dung_quy_định", "source": "tên_file"}}
    ],
    "explanation": "giải giải bằng tiếng Việt ngắn gọn"
}}
"""
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": "Bạn là chuyên gia phân tích chính sách IT Helpdesk."},
        ...
        # Giữ lại ghi chú TODO Sprint 2 gợi ý dưới đây:
        # TODO Sprint 2: Gọi LLM để phân tích phức tạp hơn
        
        return result

    except Exception as e:
        print(f"⚠️ LLM Policy Analysis failed (fallback to rules): {e}")
        # --- Rule-based fallback (giữ lại logic cũ làm dự phòng) ---
        task_lower = task.lower()
        context_lower = context_text.lower()
        exceptions_found = []

        if "flash sale" in task_lower or "flash sale" in context_lower:
            exceptions_found.append({"type": "flash_sale_exception", "rule": "Đơn hàng Flash Sale không được hoàn tiền.", "source": "policy_refund_v4.txt"})
        if any(kw in task_lower for kw in ["license", "subscription", "key"]):
            exceptions_found.append({"type": "digital_product_exception", "rule": "Sản phẩm kỹ thuật số không được hoàn tiền.", "source": "policy_refund_v4.txt"})

        policy_applies = len(exceptions_found) == 0
        policy_version_note = ""
        if "trước 01/02" in task_lower:
            policy_version_note = "Áp dụng chính sách v3."
        ...
        return {
            ...
            "explanation": "Phân tích dự phòng bằng quy tắc cơ bản.",
        }
